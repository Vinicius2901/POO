...
public class Main {
	...
	private static Animal novoAnimal() {
        ...
        try {
        	zoo.alocarAnimal(an, zoo.getSoViveiros()[vi]);
        }
	...
	private static Peixe novoPeixe() {
        ...
        try {
        	zoo.alocarAnimal( pe, zoo.getSoAquarios()[aq] );
        }
        catch( EspacoIndisponivelException e ) {
        	System.out.println(e.getMessage());
        }
	...
	private static void cadastrarAnimal() {
		if( zoo.getViveiros().length > 0 ) {
			...
			switch( opc ) {
			case 1:
				if( zoo.getSoViveiros().length > 0 ) {
					zoo.cadastrarAnimais(novoAnimal());
				}
				else {
					System.out.println("Nao ha viveiros!");
				}
				break;
			case 2:
				if( zoo.getSoAquarios().length > 0 ) {
					zoo.cadastrarAnimais(novoPeixe());
				}
				else {
					System.out.println("Nao ha aquarios!");
				}
				break;
			default:
				System.out.println("Opcao invalida!");
